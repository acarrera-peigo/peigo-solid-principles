package single_responsibility.bad.spec;

import static org.junit.Assert.assertEquals;

import java.io.ByteArrayOutputStream;
import java.io.PrintStream;
import java.util.ArrayList;
import org.junit.Test;
import single_responsibility.bad.src.BoardBad;


public class BoardBadTest {

	private final ByteArrayOutputStream outContent = new ByteArrayOutputStream();

	@Test
	public void testBoardHasNineSpots() {
		BoardBad boardBad = new BoardBad();
		assertEquals(9, boardBad.spots.size());
	}

	@Test
	public void testBoardReturnsFirstRow() {
		BoardBad boardBad = new BoardBad();
		ArrayList<String> rowOne = new ArrayList<String>();
		rowOne.add("0");
		rowOne.add("1");
		rowOne.add("2");

		assertEquals(rowOne, boardBad.firstRow());
	}

	@Test
	public void testBoardReturnsSecondRow() {
		BoardBad boardBad = new BoardBad();
		ArrayList<String> rowTwo = new ArrayList<String>();
		rowTwo.add("3");
		rowTwo.add("4");
		rowTwo.add("5");

		assertEquals(rowTwo, boardBad.secondRow());
	}

	@Test
	public void testBoardReturnsThirdRow() {
		BoardBad boardBad = new BoardBad();
		ArrayList<String> rowThree = new ArrayList<String>();
		rowThree.add("6");
		rowThree.add("7");
		rowThree.add("8");

		assertEquals(rowThree, boardBad.thirdRow());
	}

	@Test
	public void testPrintsBoardToConsole() {
		BoardBad boardBad = new BoardBad();
		System.setOut(new PrintStream(outContent));
		boardBad.display();
		assertEquals("0 | 1 | 2\n3 | 4 | 5\n6 | 7 | 8", outContent.toString());
	}
}
